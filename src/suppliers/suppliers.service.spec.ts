...
describe('SuppliersService (unit)', () => {
  ...
  it('create should throw BadRequestException when tenantId missing', async () => {
    await expect(
      suppliersService.create('', { name: 'A', phone: '123' } as any),
    ).rejects.toThrow(BadRequestException);
  });

  it('create should throw ConflictException when duplicate found', async () => {
    mockDb.partitionedFind.mockResolvedValue({
      docs: [{ _id: 't:supplier:1' }],
    });
    await expect(
      suppliersService.create('tenant1', { name: 'A', phone: '123' } as any),
  ...
  it('create should insert supplier and return result', async () => {
    ...
    mockDb.insert.mockResolvedValue({
      ok: true,
      id: 'tenant1:supplier:abc',
      rev: '1-xxx',
    });
    const res = await suppliersService.create(
      'tenant1',
      { name: 'Sup', phone: '999', email: 's@x.com' } as any,
      'creator:1',
    );
  ...
  it("create without createdBy should record log with actor 'system'", async () => {
    ...
    mockDb.insert.mockResolvedValue({
      ok: true,
      id: 'tenant1:supplier:def',
      rev: '1-yyy',
    });

    const res = await suppliersService.create('tenant1', {
      name: 'SysSup',
      phone: '111',
    } as any);
  ...
  it('findAll should return supplier docs list', async () => {
    const docs = [
      { _id: 'tenant1:supplier:1', name: 'S1' },
      { _id: 'tenant1:supplier:2', name: 'S2' },
    ];
    mockDb.partitionedFind.mockResolvedValue({ docs });
