...
export class MailService {
    ...
    private renderTemplate(templateName: string, locale: string, vars: Record<string, string>) {
        ...
        try {
            ...
            // fallback to english
            try { html = fs.readFileSync(path.join(templatesDir, `${templateName}.en.html`), 'utf8'); } catch { }
        }

        try {
            text = fs.readFileSync(txtPath, 'utf8');
        } catch (err) {
            try { text = fs.readFileSync(path.join(templatesDir, `${templateName}.en.txt`), 'utf8'); } catch { }
        }

        const render = (t: string) => t.replace(/\{\{(\w+)\}\}/g, (_, key) => vars[key] ?? '');
    ...
    async sendResetPasswordEmail(email: string, tenantId: string, token: string, locale = 'en', name?: string) {
        ...
        const subject = translations?.resetPassword?.subject || 'Reset your password';

        const { html, text } = this.renderTemplate('reset-password', locale, {
            resetUrl,
            name: name || '',
            supportEmail: this.config.get<string>('SUPPORT_EMAIL') || this.fromAddress,
        });

        // If transporter is not configured, just log and return
        if (!this.transporter) {
            this.logger.log(`=== Email (logged) ===\nTo: ${email}\nSubject: ${subject}\nText:\n${text}\nHTML:\n${html}\n=== End Email ===`);
        ...
        try {
            ...
            this.logger.log(`Fallback email content -> To: ${email}, Subject: ${subject}, URL: ${resetUrl}`);
        }
    }
}
