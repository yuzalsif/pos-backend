...
export class AccountsService {
  ...
  async create(tenantId: string, actorId: string, dto: CreateAccountDto) {
    ...
    if (initialBalance < 0)
      throw new BadRequestException('account.create.negative_initial_balance');
    const existing = await this.db.partitionedFind(tenantId, {
      selector: { name, type, currency },
    });
    if (existing.docs.length > 0)
      throw new ConflictException('account.create.duplicate');
  ...
  async deposit(
    ...
    if (!dto.categoryId)
      throw new BadRequestException('account.category_required');
  ...
  async withdraw(
    ...
    if (account.balance < dto.amount)
      throw new BadRequestException('account.withdraw.insufficient_funds');
    const now = new Date().toISOString();
    account.balance -= dto.amount;
    account.updatedBy = actorId;
    account.updatedAt = now;
  ...
  async transfer(tenantId: string, actorId: string, dto: TransferDto) {
    ...
    if (from.currency !== to.currency)
      throw new BadRequestException('account.transfer.currency_mismatch');
    if (from.balance < amount)
      throw new BadRequestException('account.transfer.insufficient_funds');
    const now = new Date().toISOString();
    ...
    try {
      ...
      await this.logs.record(
        ...
        'account',
        `${from._id}->${to._id}`,
        { fromAccountId, toAccountId, amount, categoryId },
      );
  ...
  async getTransactions(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'account.transactions.query_failed',
      );
    }
  }
}
