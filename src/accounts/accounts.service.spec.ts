...
describe('AccountsService (unit)', () => {
  ...
  it('should not create account with duplicate name', async () => {
    mockDb.partitionedFind.mockResolvedValue({
      docs: [{ _id: 'tenant1:account:xyz', name: 'NMB' }],
    });
    const dto = {
      name: 'NMB',
      initialBalance: 10000,
    ...
    await expect(
      accountsService.create('tenant1', 'user:1', dto),
    ).rejects.toThrow(ConflictException);
    await expect(
      accountsService.create('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.create.duplicate');
  });
  ...
  it('should not create account with negative initial balance', async () => {
    ...
    await expect(
      accountsService.create('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.create.negative_initial_balance');
  });

  it('should get account by id', async () => {
    mockDb.get.mockResolvedValue({
      _id: 'tenant1:account:abc',
      name: 'NMB',
      balance: 10000,
    });
  ...
  it('should throw NotFoundException if account not found', async () => {
    ...
    await expect(accountsService.get('tenant1', 'missing')).rejects.toThrow(
      NotFoundException,
    );
    await expect(accountsService.get('tenant1', 'missing')).rejects.toThrow(
      'account.not_found',
  ...
  it('should list accounts', async () => {
    const docs = [
      { _id: 'tenant1:account:1', name: 'NMB' },
      { _id: 'tenant1:account:2', name: 'CRDB' },
    ];
    mockDb.partitionedFind.mockResolvedValue({ docs });
  ...
  it('should not deposit without category', async () => {
    ...
    await expect(
      accountsService.deposit('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow(BadRequestException);
    await expect(
      accountsService.deposit('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow('account.category_required');
  });
  ...
  it('should not deposit with invalid category', async () => {
    ...
    await expect(
      accountsService.deposit('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow('account.category_invalid');
  });
  ...
  it('should not deposit with expense category', async () => {
    ...
    await expect(
      accountsService.deposit('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow('account.category_type_mismatch');
  });
  ...
  it('should not withdraw without category', async () => {
    ...
    await expect(
      accountsService.withdraw('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow(BadRequestException);
    await expect(
      accountsService.withdraw('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow('account.category_required');
  ...
  it('should not withdraw more than balance', async () => {
    ...
    await expect(
      accountsService.withdraw('tenant1', 'user:1', 'abc', dto),
    ).rejects.toThrow('account.withdraw.insufficient_funds');
  });
  ...
  it('should transfer money atomically between accounts and log', async () => {
    ...
    const toAccount = {
      _id: 'tenant1:account:def',
      name: 'CRDB',
      balance: 5000,
      currency: 'TZS',
    };
    const category = {
      _id: 'tenant1:category:cat3',
      name: 'Transfer',
      type: 'expense',
    };
    mockDb.get.mockImplementation((id: string) => {
  ...
  it('should not transfer without category', async () => {
    ...
    await expect(
      accountsService.transfer('tenant1', 'user:1', dto),
    ).rejects.toThrow(BadRequestException);
    await expect(
      accountsService.transfer('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.category_required');
  ...
  it('should not transfer if currencies do not match', async () => {
    ...
    await expect(
      accountsService.transfer('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.transfer.currency_mismatch');
  });
  ...
  it('should not transfer more than available in from account', async () => {
    ...
    await expect(
      accountsService.transfer('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.transfer.insufficient_funds');
  });
  ...
  it('should not transfer if either account does not exist', async () => {
    ...
    await expect(
      accountsService.transfer('tenant1', 'user:1', dto),
    ).rejects.toThrow('account.not_found');
  });
  ...
  it('should rollback transfer and delete transactions if transaction creation fails', async () => {
    ...
    mockDb.get.mockImplementation((id: string) => {
      ...
      if (id.includes(':transaction:'))
        return Promise.resolve({ _id: id, _rev: '1-abc' });
      return Promise.reject({ statusCode: 404 });
    });
