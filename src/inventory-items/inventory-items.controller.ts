...
export class InventoryItemsController {
    ...
    async create(@Req() req, @Body() createInventoryItemDto: CreateInventoryItemDto) {
        const { tenantId, userId, userName } = req.user;
        return this.inventoryItemsService.create(tenantId, userId, userName, createInventoryItemDto);
    }
    ...
    @RequirePermissions(Permission.PRODUCTS_VIEW)
    async findAvailableByProduct(@Req() req, @Param('productId') productId: string) {
        const { tenantId } = req.user;
        return this.inventoryItemsService.findAvailableByProduct(tenantId, productId);
    }
    ...
    async update(
        ...
        const { tenantId, userId, userName } = req.user;
        return this.inventoryItemsService.update(tenantId, userId, userName, id, updateInventoryItemDto);
    }
}
