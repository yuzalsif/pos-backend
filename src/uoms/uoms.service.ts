...
export class UomsService {
  ...
  async create(tenantId: string, userId: string, dto: CreateUomDto) {
    try {
      ...
      const all = await this.db.partitionedFind(tenantId, {
        selector: { type: 'uom' },
      });
      ...
      if (dto.baseUnit) {
        ...
        const baseId = dto.baseUomId.includes(':')
          ? dto.baseUomId
          : `${tenantId}:uom:${dto.baseUomId}`;
        ...
        try {
          ...
          throw new BadRequestException({
            key: 'uom.base_not_found',
            vars: { baseUomId: dto.baseUomId },
          });
        }
      ...
      const uomDoc = {
        ...
        name: dto.name,
        toBaseFactor:
          typeof dto.toBaseFactor === 'number'
            ? dto.toBaseFactor
            : dto.baseUnit
              ? 1
              : 1,
