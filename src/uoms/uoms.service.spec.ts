...
describe('UomsService (unit)', () => {
  ...
  it('create non-base should throw BadRequestException when baseUomId not found', async () => {
    ...
    const dto: CreateUomDto = {
      ...
      name: 'Box',
      baseUnit: false,
      baseUomId: 'nonexistent',
      toBaseFactor: 10,
    } as any;

    await expect(uomsService.create('tenant1', 'user:2', dto)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('findAll should return list of uoms', async () => {
    const docs = [
      { _id: 'tenant1:uom:1', code: 'pc' },
      { _id: 'tenant1:uom:2', code: 'box' },
    ];
  ...
  it('create non-base should succeed when baseUomId exists and normalize baseUomId', async () => {
    ...
    mockDb.get.mockImplementation(async (id: string) => {
      if (id === 'tenant1:uom:base')
        return { _id: 'tenant1:uom:base', baseUnit: true };
      return {};
    });
