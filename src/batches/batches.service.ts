...
export class BatchesService {
    ...
    async create(
        ...
        try {
            ...
            const productId = createDto.productId.includes(':')
                ? createDto.productId
                : `${tenantId}:product:${createDto.productId}`;
            let product: any;
            try {
                ...
                if ((err as any).statusCode === 404) {
                    throw new NotFoundException({
                        ...
                        vars: { productId: createDto.productId },
                    });
                }
                throw err;
            }
            ...
            if (existing.docs.length > 0) {
                throw new ConflictException({
                    key: 'batch.already_exists',
                    vars: { batchNumber: createDto.batchNumber },
                });
            }
            ...
            if (
                !createDto.priceOverride &&
                product.unitsOfMeasure &&
                product.unitsOfMeasure.length > 0
            ) {
            ...
            try {
                ...
                if (this.logsService) {
                    try {
                        ...
                        this.logger.warn('Failed to record log for batch creation', logError);
                ...
                if (response && !stockAdjusted) {
                    // Stock adjustment failed after batch was created - ROLLBACK
                    this.logger.error(
                        `Rolling back batch creation for ${createDto.batchNumber} due to stock adjustment failure`,
                        error,
                    );
                    ...
                    try {
                        ...
                        this.logger.log(
                            `Successfully rolled back batch ${createDto.batchNumber}`,
                        );
                    } catch (rollbackError) {
                        this.logger.error(
                            `CRITICAL: Failed to rollback batch ${createDto.batchNumber}. Manual intervention required.`,
                            rollbackError,
                        );
                    }
    ...
    async findAvailable(tenantId: string, productId: string) {
        try {
            const fullProductId = productId.includes(':')
                ? productId
    ...
    async findAll(tenantId: string, productId?: string) {
        try {
            ...
            if (productId) {
                selector.productId = productId.includes(':')
                    ? productId
                    : `${tenantId}:product:${productId}`;
            }
