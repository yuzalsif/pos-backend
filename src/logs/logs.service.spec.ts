...
describe('LogsService (unit)', () => {
    ...
    it('record should insert a log document and return id/rev', async () => {
        ...
        const res = await logsService.record('tenant1', { userId: 'u:1', name: 'U' }, 'something.create', 'something', 'tenant1:something:1', { foo: 'bar' });
    ...
    it('record should throw InternalServerErrorException when insert fails', async () => {
        ...
        await expect(logsService.record('tenant1', { userId: 'u:2' }, 'a', 'b')).rejects.toThrow(InternalServerErrorException);
    });
    ...
    it('findAll should query partitionedFind with provided filters and return docs', async () => {
        ...
        const result = await logsService.findAll('tenant1', { action: 'a', resource: 'r', userId: 'u:1', limit: 10, skip: 0 });

        expect(mockDb.partitionedFind).toHaveBeenCalledTimes(1);
        const q = mockDb.partitionedFind.mock.calls[0][1] || mockDb.partitionedFind.mock.calls[0][0];
    ...
    it('findAll should throw InternalServerErrorException on db error', async () => {
        ...
        await expect(logsService.findAll('tenant1')).rejects.toThrow(InternalServerErrorException);
    });
});
