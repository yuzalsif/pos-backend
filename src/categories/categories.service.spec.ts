...
describe('CategoriesService (unit)', () => {
    ...
    it('should create income category', async () => {
        ...
        expect(mockLogs.record).toHaveBeenCalledWith('tenant1', { userId: 'user:1' }, 'category.create', 'category', expect.any(String), { name: 'Sales', type: 'income' });
    });

    it('should create expense category', async () => {
        mockDb.partitionedFind.mockResolvedValue({ docs: [] });
        mockDb.insert.mockResolvedValue({ id: 'tenant1:category:def', rev: '1-0' });
        const dto = { name: 'Rent', type: 'expense' as const, description: 'Monthly rent payments' };
        const res = await categoriesService.create('tenant1', 'user:1', dto);
    ...
    it('should not create duplicate category', async () => {
        mockDb.partitionedFind.mockResolvedValue({ docs: [{ _id: 'tenant1:category:xyz', name: 'Sales', type: 'income' }] });
        const dto = { name: 'Sales', type: 'income' as const };
        await expect(categoriesService.create('tenant1', 'user:1', dto)).rejects.toThrow(ConflictException);
        await expect(categoriesService.create('tenant1', 'user:1', dto)).rejects.toThrow('category.create.duplicate');
    });

    it('should create subcategory under parent', async () => {
        const parent = { _id: 'tenant1:category:parent', name: 'Income', type: 'income' };
        mockDb.partitionedFind.mockResolvedValue({ docs: [] });
        mockDb.get.mockResolvedValue(parent);
        mockDb.insert.mockResolvedValue({ id: 'tenant1:category:child', rev: '1-0' });
        const dto = { name: 'Consulting', type: 'income' as const, parentCategoryId: 'parent' };
        const res = await categoriesService.create('tenant1', 'user:1', dto);
    ...
    it('should not create subcategory with different type than parent', async () => {
        ...
        mockDb.partitionedFind.mockResolvedValue({ docs: [] });
        mockDb.get.mockResolvedValue(parent);
        const dto = { name: 'Consulting', type: 'expense' as const, parentCategoryId: 'parent' };
        await expect(categoriesService.create('tenant1', 'user:1', dto)).rejects.toThrow(BadRequestException);
        await expect(categoriesService.create('tenant1', 'user:1', dto)).rejects.toThrow('category.create.parent_type_mismatch');
    });

    it('should get category by id', async () => {
        mockDb.get.mockResolvedValue({ _id: 'tenant1:category:abc', name: 'Sales', type: 'income' });
    ...
    it('should throw NotFoundException if category not found', async () => {
        ...
        await expect(categoriesService.get('tenant1', 'missing')).rejects.toThrow(NotFoundException);
        await expect(categoriesService.get('tenant1', 'missing')).rejects.toThrow('category.not_found');
    });
    ...
    it('should update category', async () => {
        const category = { _id: 'tenant1:category:abc', name: 'Sales', type: 'income' };
    ...
    it('should not update to duplicate name', async () => {
        const category = { _id: 'tenant1:category:abc', name: 'Sales', type: 'income' };
        const existing = { _id: 'tenant1:category:def', name: 'Consulting', type: 'income' };
        mockDb.get.mockResolvedValue(category);
        mockDb.partitionedFind.mockResolvedValue({ docs: [existing] });
        const dto = { name: 'Consulting' };
        await expect(categoriesService.update('tenant1', 'user:1', 'abc', dto)).rejects.toThrow(ConflictException);
        await expect(categoriesService.update('tenant1', 'user:1', 'abc', dto)).rejects.toThrow('category.update.duplicate');
    });

    it('should not delete category with subcategories', async () => {
        const category = { _id: 'tenant1:category:parent', name: 'Income', type: 'income', _rev: '1-0' };
        mockDb.get.mockResolvedValue(category);
        mockDb.partitionedFind.mockResolvedValue({
            docs: [{ _id: 'tenant1:category:child', parentCategoryId: category._id }]
        });
        await expect(categoriesService.delete('tenant1', 'user:1', 'parent')).rejects.toThrow(BadRequestException);
        await expect(categoriesService.delete('tenant1', 'user:1', 'parent')).rejects.toThrow('category.delete.has_subcategories');
    }); it('should not delete category used in transactions', async () => {
        const category = { _id: 'tenant1:category:abc', name: 'Sales', type: 'income', _rev: '1-0' };
        mockDb.get.mockResolvedValue(category);
        ...
        expect(mockLogs.record).toHaveBeenCalledWith('tenant1', { userId: 'user:1' }, 'category.delete', 'category', category._id, { name: 'Sales' });
    });

    it('should get subcategories', async () => {
        const parent = { _id: 'tenant1:category:parent', name: 'Income', type: 'income' };
        const subcategories = [
            { _id: 'tenant1:category:child1', parentCategoryId: parent._id, name: 'Sales' },
            { _id: 'tenant1:category:child2', parentCategoryId: parent._id, name: 'Consulting' }
        ];
        mockDb.get.mockResolvedValue(parent);
    ...
    it('should build category tree', async () => {
        const categories = [
            { _id: 'tenant1:category:1', name: 'Income', type: 'income', parentCategoryId: null },
            { _id: 'tenant1:category:2', name: 'Sales', type: 'income', parentCategoryId: 'tenant1:category:1' },
            { _id: 'tenant1:category:3', name: 'Consulting', type: 'income', parentCategoryId: 'tenant1:category:1' },
        ];
