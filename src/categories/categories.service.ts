...
export class CategoriesService {
  ...
  async create(tenantId: string, actorId: string, dto: CreateCategoryDto) {
    ...
    if (existing.docs.length > 0)
      throw new ConflictException('category.create.duplicate');
    ...
    const category = {
      ...
      parentCategoryId: parentCategoryId
        ? `${tenantId}:category:${parentCategoryId}`
        : null,
      description,
      createdBy: actorId,
      updatedBy: actorId,
  ...
  async update(
    ...
    if (dto.name) {
      ...
      const duplicates = existing.docs.filter(
        (doc: any) => doc._id !== category._id,
      );
      if (duplicates.length > 0)
        throw new ConflictException('category.update.duplicate');
  ...
  async delete(tenantId: string, actorId: string, categoryId: string) {
    ...
    const usedTransactions = transactions.docs.filter((doc: any) =>
      doc._id.includes(':transaction:'),
    );
  ...
  async getSubcategories(tenantId: string, categoryId: string) {
    try {
      ...
      throw new InternalServerErrorException(
        'category.subcategories.query_failed',
      );
    }
  }
